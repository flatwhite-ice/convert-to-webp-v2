...
- **완료 요약** — 종료 시 "전체 N개 중 M개 변환 완료" + 저장 위치를 메시지 박스로 표시.
...
- **onedir 패키징** (`build.py`) — `--onedir --windowed --noupx`. 실행 폴더(`dist/convert-to-webp2/`: `exe` + `_internal/`)를 생성하고 `dist/convert-to-webp2-<os>.zip`으로 자동 압축.
